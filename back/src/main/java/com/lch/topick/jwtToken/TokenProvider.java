...
public class TokenProvider {
	...
	/* 깃허브에 올라갈 코드 내부에 시크릿 키가 공개되어 있으면 보안상 위험, 
	 * 때문에 환경변수로 등록 후 불러오는 방식 사용
	...
	// 2. Role 을 Token 에 포함한 공통JWT Token 생성 메서드
	/* 	  accessToken, refreshToken 공통 메서드
	 * 	  claimList(memberId, roleList), 만료시간 expMillis 받음 */
	private String createToken(Map<String, Object> claimList, long expMillis) {
		
		SecretKey key = getSigningKey();
		Instant now = Instant.now(); 						// 토큰 발급 시간
		Instant exp = now.plusMillis(expMillis);			// 토큰 만료 시간
	...
	// 4. refreshToken 발급
	public String createRefreshToken(Map<String, Object> claimList) {
		return createToken(claimList, refreshTokenExp);
	}
	...
	public Claims validateToken(String token) {
		...
		// 5.1 JSON 데이터의 Payload(Claims) 에 해당되는 부분을 return
		...
		// 5.2 오류 처리
	        ...
	        throw new CustomException(ErrorCode.JWT_EXPIRED);   //만료된 토큰
	
	    } catch (MalformedJwtException e) {
	        throw new CustomException(ErrorCode.JWT_MALFORMED); //잘못된 형식
	
	    } catch (JwtException e) {
	        throw new CustomException(ErrorCode.JWT_ERROR); 	//검증 중 오류 발생
	    } //try
