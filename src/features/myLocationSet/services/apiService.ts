...
// 0. 전송 옵션과 데이터 구조를 위한 인터페이스 정의 (Java의 DTO/VO 관점)
interface AxiosOptions extends AxiosRequestConfig {
  url: string;
  method: string;
  headers: Record<string, string>; // Key, Value가 모두 string인 객체
}

// 1. axios 요청 함수 
// => 요청시 필요한 정보를 매개변수로 전달받음
// => ACCESS_TOKEN 도 매개변수로 전달함
export async function apiCall(url: string, method: string, requestData: any, token: string | null) {
